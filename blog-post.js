...
async function loadPost(postId) {
  try {
    ...
    // 1. Fetch all posts from the proxy
    ...
    // 2. Find the specific post by ID
    ...
    currentPost = {
      ...post,
      // Ensure date is a formatted string or Date object as your UI expects
      publish_date: post.publish_date || new Date().toISOString()
    };
    
    console.log('📄 Post loaded successfully');
    
    // 4. Update the UI using your existing display function
    displayPost(currentPost);
    
    // 5. Load related posts from the data we already fetched
    if (typeof loadRelatedPosts === 'function') {
      await loadRelatedPosts(currentPost.category, postId, allPostsData);
    }
