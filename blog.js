...
async function loadBlogPosts() {
  try {
    console.log('🔍 Fetching posts from CMS Proxy...');
    ...
    // The proxy returns the array directly
    const data = await response.json();
    
    // 2. Transform the data to match your UI needs
    blogPosts = data
      .filter(post => post.published === true) // Only show published
      .map(post => ({
        id: post.id,
        ...
        // Map the category label
        categoryLabel: (typeof categoryLabels !== 'undefined' && categoryLabels[post.category]) 
                       ? categoryLabels[post.category] 
                       : 'Career Advice',
        // Formatting the date for your sort logic
        date: post.publish_date ? post.publish_date : new Date().toISOString().split('T')[0],
        readTime: post.read_time || '5 min read',
        image: post.featured_image || post.image || 'https://images.unsplash.com/photo-1586281380349-632531db7ed4?w=800&q=80',
    ...
    // 4. Update UI - wrapped in checks to prevent errors if elements are missing
    const totalArticlesEl = document.getElementById('totalArticles');
    if (totalArticlesEl) totalArticlesEl.textContent = blogPosts.length;
    
    console.log('✅ Successfully loaded', blogPosts.length, 'posts');

    // 5. Trigger your existing render functions
    renderFeaturedPost();
    renderBlogGrid();
    ...
    // Fail-safe UI updates
    const totalArticlesEl = document.getElementById('totalArticles');
    if (totalArticlesEl) totalArticlesEl.textContent = '0';
    
    const blogGrid = document.getElementById('blogGrid');
    if (blogGrid) {
      blogGrid.innerHTML = '<p style="text-align:center; padding: 2rem;">Unable to load posts at this time.</p>';
    }
  }
}
